...
modules. The module class does some simple, yet convenient things:

- It stores the `appId` used for
...
- It enables you to use the re-use references for similar outputs in
  different modules. The `ShinyModule` will add the correct namespacing
  for you when you use
  [`shiny::NS()`](https://rdrr.io/pkg/shiny/man/NS.html)

## Overrides

When inheriting the `ShinyModule` class it is expected that you override
the `UI()` and / or `server()` methods. The `UI()` method should return
something akin to a
[`shiny::tagList()`](https://rstudio.github.io/htmltools/reference/tagList.html).
By default they are defined, but do nothing. This allows for just
rendering simple text in a module, without any back-end like the
[`DarwinShinyModules::Text`](https://darwin-eu-dev.github.io/DarwinShinyModules/reference/Text.md)
module. Or a static image.
...
with a `UI` and a `server` method. For a new module you can either
directly add functionality to these functions, and use `private` and
`active` fields and methods within the `R6` framework. Or if you are
more familiar with functions, you may want to call your own `server` and
...
Let’s remake `MyModule` putting all the functionality directly on the
module, and see what extra funcionality and information exists on the
module. In this module I want to:

- Display the [`head()`](https://rdrr.io/r/utils/head.html) of the
  `iris` data frame, using `"myTable"` as the `input` / `output`
  reference.
- Display the `appId`, `moduleName`, and `instanceId`, using
...
To initialize our module object we call the
[`new()`](https://rdrr.io/r/methods/new.html) method on our defined
`MyModule`:
...
To see what the module contains we can simply
...
There are several items listed under Public: `appId`, `clone()`, `data`,
`id()` [`initialize()`](https://rdrr.io/r/methods/new.html),
`instanceId`, `moduleName`, `server()`, `UI()`, and
[`validate()`](https://rdrr.io/pkg/shiny/man/validate.html). Note that
`appId`, `data`, `instanceId`, and `moduleName` have a Private counter
parts: `.appId`, `.data`, `.instanceId`, `moduleName`.
...
Notice that if we initialize multiple instances of the same type of
...
We can make a new module called: `MyModule2` which inherits from
`MyModule`. I want the module to be exactly the same, but instead of
...
Modules may be nested in other modules. The nested modules can
communicate data to one another.

Let’s conciser the following plot function, for the `iris` dataset. We
add an arbitrary size column for this example, which we will manipulate
...
With our modules added, we can call their respective `server()` and
...
The `Table` and `PlotStatic` modules are now independent from one
another. However, we can change that by adding some extra code in the
`.server()` method.
...
We can see that if we select rows in our table, the size updates in the
plot for the selected row.
