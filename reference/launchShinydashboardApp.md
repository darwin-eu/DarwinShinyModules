...
``` r
launchShinydashboardApp(appStructure, title = NULL, async = FALSE)
```
...
- appStructure:
  ...
  (`list(list())`) A list of named lists, containing modules. The level
  of nesting groups or separates modules in menu items `"_"` will be
  read as a space.
...
- async:
  ...
  (`logical(1)`: `FALSE`) Run app asynchronously
